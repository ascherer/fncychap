...
\begin{document}
  ...
  \label{sec:TW}
    ...
    by the command\sk\\
    \nsp\fbox{\A{ChRuleWidth}\texttt{\textbraceleft}{\em
    width}\texttt{\textbraceright}}\sk\\
    ...
    \nsp\fbox{\A{ChNameVar}\texttt{\textbraceleft}{\em stuff}\texttt{\textbraceright},
      \A{ChNumVar}\texttt{\textbraceleft}{\em stuff}\texttt{\textbraceright} and
      \A{ChTitleVar}\texttt{\textbraceleft}{\em stuff}\texttt{\textbraceright}}\sk\\
    related to the chapter name number and title respectively.
    The argument, {\em stuff}, to these functions can be for example
    \mbox{\A{ChNameVar}\texttt{\textbraceleft}\A{huge}\A{rmfamily}\A{centering}%
    \texttt{\textbraceright}}.
...
\noindent\textbf{Note:} An alternative version of this chapter head exist
...
\noindent\textbf{Note:} It appears as if the rendering in YAP (dvi previewer in
MikTeX) differs from dvips in that the gray box become foreground. Thus, the
chapter number is partly hidden.

  \chapter{A simple example}
    \lettrine{I}{f the pre-defined styles} do not fulfill your needs
    then you can modify the formating routines. The formatting is
    controlled by three commands. One might as well redefine the original
    ...
    \nsp\fbox{\A{DOTI}\texttt{\textbraceleft}{\#1}\texttt{\textbraceright}}\sk\\
    and\sk\\
    \nsp\fbox{\A{DOTIS}\texttt{\textbraceleft}{\#1}\texttt{\textbraceright}}\sk\\
    ...
    respectively. Finally, the functions \A{FmN}\verb+{ }+ and
    \A{FmTi}\verb+{ }+ acts accordingly to \A{Ch***AsIs}, \A{Ch***UpperCase}
    and \A{Ch***LowerCase}. Note that the stars indicate appropriate
